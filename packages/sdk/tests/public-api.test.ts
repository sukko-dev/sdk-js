// Public-surface snapshot (no dead code). Pins the exported runtime surface so a member
...
// (unmodelled) is asserted absent by scanning the model source, since a type cannot be probed
// at runtime.
...
describe("public API — CLOSE_CODES", () => {
	it("carries the contract set plus the SDK-internal UNAUTHORIZED, and nothing off-contract", () => {
		...
		// The disposition removed these off-contract, unemitted codes (§I exact-match).
...
describe("public API — SukkoClient methods", () => {
	...
	it("no longer exposes the removed recovery/reconnect methods (internalized)", () => {
...
describe("public API — removed type exports stay gone", () => {
...
describe("message models — `seq` is not modelled", () => {
