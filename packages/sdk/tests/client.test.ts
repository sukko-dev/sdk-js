...
describe("SukkoClient", () => {
	...
	describe("reconnection", () => {
		...
		it("reconnects on going_away (1001) — transient", async () => {
			const transport = new MockTransport();
		...
		it("routes reconnect backoff + jitter through the injected clock", async () => {
	...
	describe("recovery routing", () => {
		...
		it("drops a malformed gap (missing last_pos or channel) — no replay, kept off messages()", async () => {
			...
			// dropped: no `replay` sent, and neither surfaced to the delivery stream (§II).
