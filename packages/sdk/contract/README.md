...
CI checks out only this repo, so the contract-coverage test (`coverage.test.ts`) cannot
...
only (it never ships in the runtime bundle — the SDK ships with zero runtime dependencies).
...
The parity-vector fixtures under `vectors/` are **canonical here** — the Python SDK vendors a
