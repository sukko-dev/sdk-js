...
export class SukkoClient extends TypedEventEmitter<SukkoClientEvents> {
	...
	// Subscriptions — desired-vs-granted state. `desired` is what the caller asked for; the
	...
	// Timing seam — injectable via options; defaults to SystemClock (its setTimeout/Date.now
	...
	disconnect(): void {
		...
		// close vs reconnect-epoch lifecycle (queue survival across epochs) is defined by a later
	 ...
	 * channels the api-key was not granted (the not-granted delta). While disconnected it stores
	...
	async escalate(jwt: string): Promise<void> {
		...
		// escalation is by stream reconnect with the new JWT (spec.md §240). Store the credential,
	...
	private handleTransportOpen(): void {
		...
		// silently resuming live-only (the no-silent-drop invariant on the SSE path).
		if (!this.transport.capabilities.canSend) {
	...
	private handleTransportClose(code: number, reason: string): void {
		...
		// reconnect with backoff. Gated on `!canSend` — only SSE emits this; a WS
		...
		try {
			...
			switch (raw.type) {
				...
				case "gap": {
					...
					// frame (the missing key silently dropped by JSON.stringify). Drop-and-continue (§II).
			...
			// Malformed (non-JSON) frame — drop and continue; the read-pump must survive it.
		}
	}
	...
	private async handleReconnect(): Promise<void> {
		...
		// gives up and enters the terminal `error` state — no indefinite retry.
	 ...
	 * teardown + the reconnect path directly here (a local 4000 close → reconnect).
	 */
	...
	private setupNetworkListeners(): void {
		...
		// (import safety). `online` needs `window`; `visibilitychange` needs `document`.
