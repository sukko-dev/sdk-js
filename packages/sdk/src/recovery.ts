// Reconnect / gap-recovery engine — the pos-anchored recovery state
...
// the machine is directly parity-vector-drivable (actions are the canonical encoding).
//
...
// frame-reset idle timer** (measures server silence, not consumer slowness), and
// `buildReconnect()` **probes with an empty `last_pos`** once connected-once so a pure-Direct session
// actually elicits `not_available` and degrades instead of silently dropping.
...
export class RecoveryEngine {
	...
	/** Mark that the client has completed at least one connection — arms the reconnect probe. */
	markConnected(): void {
	 ...
	 * map) so a pure-Direct session elicits `not_available` and degrades. Returns null only on
