...
export const SUKKO_DEFAULTS = {
	...
	// Delivery-queue defaults (back-pressure). bufferSize must be >= historyLimit +
	// maxReplayMessages so a recovery burst fits without tripping back-pressure.
	bufferSize: 256,
	historyLimit: 100,
	maxReplayMessages: 100,
	overflowPolicy: "drop_oldest",
	// Recovery. replayFloorMs = per-channel auto-replay rate limit (server WS_REPLAY_RATE_LIMIT_INTERVAL);
	// recoveryDeadlineMs = 2× server WS_REPLAY_TIMEOUT, an idle timer reset on each recovery frame.
	replayFloorMs: 10000,
	recoveryDeadlineMs: 10000,
	// Auth refresh. authRefreshFloorMs = min interval between refresh sends (floors an
	...
	// SSE. sseIdleTimeoutMs = max silence (no bytes, incl. `: keepalive` comments) before the
...
export const CLOSE_CODES = {
	...
	/** Remote (operator-initiated) close: server force-disconnected this client. Terminal — no auto-reconnect. */
