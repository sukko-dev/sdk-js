...
| [`@sukko/websocket-node`](packages/websocket-node) | WebSocket transport adapter for Node | [![npm](https://img.shields.io/npm/v/@sukko/websocket-node)](https://www.npmjs.com/package/@sukko/websocket-node) |
...
- **Transport-agnostic core** — `@sukko/sdk` has no transport dependency; use the WebSocket adapter or supply your own
...
belongs here, **open an issue first** — if we agree it is right, we will say so
and sort out the contribution terms then. Bug reports, documentation problems and
questions about behaviour are always welcome.

Changes we have agreed to include a changeset: `bunx changeset`.
